...
export async function GET() {
  try {
    ...
    const pendingTransactions = txStats?.filter(t => t.status === 'PENDING' || t.status === 'DEPOSIT_REQUESTED').length || 0;
    const completedTransactions = txStats?.filter(t => t.status === 'COMPLETED' || t.status === 'PAYOUT_COMPLETED').length || 0;
    ...
    const activities = [
        ...(recentKYC?.map(k => ({
            ...
            subtitle: (Array.isArray(k.client_profiles) ? k.client_profiles[0]?.company_name : (k.client_profiles as any)?.company_name) || 'New Client',
            time: k.createdAt,
            status: k.status,
            color: k.status === 'approved' ? 'text-green-600' : k.status === 'rejected' ? 'text-red-600' : 'text-blue-600',
            bg: k.status === 'approved' ? 'bg-green-100' : k.status === 'rejected' ? 'bg-red-100' : 'bg-blue-100',
            iconType: 'kyc'
        })) || []),
        ...(recentTxs?.map(t => ({
            id: t.id,
            ...
            subtitle: (Array.isArray(t.client_profiles) ? t.client_profiles[0]?.company_name : (t.client_profiles as any)?.company_name) || 'Client',
            time: t.createdAt,
            status: t.status,
